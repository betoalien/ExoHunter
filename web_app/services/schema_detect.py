...
def _load_default_expected_map() -> Dict[str, List[str]]:
    ...
    return {
        ...
        "koi_teq": ["equilibrium_temp", "teq", "pl_eqt"],
        "koi_insol": ["insolation", "flux", "pl_insol"],
        "koi_smass": ["stellar_mass", "st_mass"],
        "koi_srad": ["stellar_radius", "st_rad"],
        "koi_slogg": ["logg", "st_logg"],
        "koi_steff": ["teff", "st_teff"],
        "koi_disposition": ["disposition", "pl_disposition"],
        "koi_duration": ["duration_hours", "tdur"],
...
def preview_columns(path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        out["columns"] = list(df.columns)
        out["format"] = "csv"
        # rows se desconoce en lectura parcial; mejor None
        return out
    except Exception:
        return out
...
def normalize_columns(df_in):
    ...
    # Si no existe object_id, intenta sintetizarlo
    if "object_id" not in df.columns:
        # Prefer KOI name si existe (después del rename)
